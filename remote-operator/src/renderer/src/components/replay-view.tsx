...
function DropZone({ onOpen, onDrop, isLoading }: {
  onOpen: () => void
  ...
  return (
    ...
    >
      <div
        className={cn(
          'absolute inset-4 rounded-xl border-2 border-dashed transition-colors',
          isDragging ? 'border-cyan-500/60 bg-cyan-500/5' : 'border-white/[0.08]'
        )}
      />
      <div className="text-center relative z-10">
        <div className={cn(
          ...
          <svg className={cn('w-8 h-8 transition-colors', isDragging ? 'text-cyan-500/60' : 'text-white/20')} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <p className="text-white/20 text-xs mt-1">Replay recorded episodes</p>
      </div>
    </div>
  )
}
...
function PlaybackBar({ replay }: { replay: ReplayState }) {
  ...
  const progress = replay.totalFrames > 1
    ...
    <div className="absolute bottom-0 left-0 right-0 z-20">
      <div className="bg-gradient-to-t from-black/80 to-transparent pt-12 pb-4 px-4">
        <div className="flex items-center gap-3">
          ...
          {/* Prev/Next */}
          <button onClick={replay.prevFrame} className="text-white/40 hover:text-white/70 transition-colors cursor-pointer">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
            </svg>
          </button>
          <button onClick={replay.nextFrame} className="text-white/40 hover:text-white/70 transition-colors cursor-pointer">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          >
            <div
              className="h-full bg-cyan-500 rounded-full transition-[width] duration-75 relative"
...
export function ReplayView({ replay }: ReplayViewProps) {
  ...
  }, [replay.isLoaded, replay.togglePlayback, replay.nextFrame, replay.prevFrame, replay.close])

  if (!replay.isLoaded) {
    return (
      <div className="flex-1 relative overflow-hidden mx-3 mb-3 rounded-lg border border-white/[0.06] bg-white/[0.02]">
        <DropZone
  ...
  return (
    <div className="flex-1 relative overflow-hidden mx-3 mb-3 rounded-lg border border-white/[0.06] bg-black">
      {/* Close button */}
      <button
        onClick={replay.close}
        className="absolute top-4 left-4 z-30 w-8 h-8 flex items-center justify-center rounded-lg bg-black/40 backdrop-blur-md border border-white/[0.08] hover:border-white/20 transition-colors cursor-pointer"
      >
