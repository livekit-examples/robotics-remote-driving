...
export function RecordButton({ isRecording, frameCount, elapsed, onStart, onStop }: RecordButtonProps) {
  if (isRecording) {
    return (
      <button
        onClick={onStop}
        className="absolute top-5 right-5 z-10 flex items-center gap-3 px-4 py-2.5 rounded-[3px] bg-black/60 backdrop-blur-md border border-red-500/30 hover:border-red-500/50 transition-all cursor-pointer group"
      >
  ...
  return (
    <button
      onClick={onStart}
      className="absolute top-5 right-5 z-10 flex items-center gap-2.5 px-4 py-2.5 rounded-[3px] bg-black/40 backdrop-blur-md border border-white/[0.08] hover:border-white/20 hover:bg-black/60 transition-all cursor-pointer"
    >
