    ...
    <div
      className={cn(
        ...
        active
          ...
          : "backdrop-blur-md bg-black/30 border-white/[0.08] text-white/25",
        className,
      )}
    >
      {label}
...
export function KeyOverlay({ held, className }: KeyOverlayProps) {
  return (
    <div className={cn("absolute bottom-6 left-6 z-10", className)}>
      <div className="flex flex-col items-center gap-2">
        {/* W key */}
        <div className="flex justify-center">
          <KeyCap
            label="W"
            active={held.has(BUTTONS.UP)}
            className="w-12 h-10"
          />
        </div>
        ...
        <div className="flex gap-2">
          <KeyCap
            label="A"
            active={held.has(BUTTONS.LEFT)}
            className="w-12 h-10"
          />
          <KeyCap
            label="S"
            active={held.has(BUTTONS.DOWN)}
            className="w-12 h-10"
          />
          <KeyCap
            label="D"
            active={held.has(BUTTONS.RIGHT)}
            className="w-12 h-10"
          />
        </div>
